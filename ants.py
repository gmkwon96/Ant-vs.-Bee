    ...
    def buff(self):
        ...
        if self.is_buffed == False:
            ...
            while True:
                ...
                if place.ant and place.ant.is_container and place.ant.ant_contained:
                    if place.ant.ant_contained not in self.doubled_ants:
                        place.ant.ant_contained.damage *= 2
                        self.doubled_ants.append(place.ant.ant_contained)
                self.is_buffed == True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not curr.is_hive:
            if curr.bees and self.min_range <= counter <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in bee_list:
            bee.reduce_health(bee, reflected_damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if not self.chew_countdown:
            random_bees = random_bee(self.place.bees)
            if random_bees:
                random_bees.reduce_health(random_bees.health)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.backwards:
            ...
            self.backwards = False
        # if bee's direction is -1, 
            # if bee is already right in front of the Hive, then
                    #don't move (manipulate destination to not move this bee)
            # otherwise
                #  then change the destination to be one space toward the Hive
        #
        if self.blocked():
    ...
    def slow(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self. already_slowed:
            length +=3
        def slow_action(gamestate):
            if gamestate.time %2 ==0 and length >0: # it can only move on when gametime is even and length > 0
                self.action(gamestate) # perform the bee's original action
    ...
    def scare(self, length):
        ...
        def scare_action(gamestate):
            if self.move_to(self.place.entrance) # if a bee is next to the Hive, it should not move
                length -=1
            else:
            # if a bee is slowed and gametime is odd they cannot back away
                self.move_to(self.place.entrance)
                length-=1
            # change the bee's direction to be -1
            # perform bee's action 
            # change the bee's direction back to +1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target and not target.scared_before: 
            target.scare(2)
            target.scared_before = True # target has not been scared before
        # END Problem EC
